...
import { DigitizedDoc, DOC_TYPE_LABELS, ExtractedField, LineItem } from "../../core/types/document.types";
import { ApiError } from "../../core/types/common.types";
import { docApi } from "../../api/docApi";
import { toast } from "sonner";
import { DocumentCanvas } from "./components/DocumentCanvas";
import { IconAlertTriangle, IconPencil, IconCheck, IconScan, IconArrowsMaximize } from "../../components/icons";
...
export function normalizeDocTypeLabel(value: string): string {
  ...
  if (
    normalizedKey.includes("hop_dong_dich_vu") ||
    normalizedKey.includes("contract_service") ||
    normalizedKey.includes("service_contract")
  ) {
    return "Hợp đồng dịch vụ";
  }
  if (
    normalizedKey.includes("hop_dong_hang_hoa") ||
    normalizedKey.includes("contract_good") ||
    normalizedKey.includes("goods_contract")
  ) {
  ...
  if (
    normalizedKey.includes("to_trinh_dich_vu") ||
    normalizedKey.includes("proposal_service") ||
    normalizedKey.includes("service_proposal")
  ) {
    return "Tờ trình dịch vụ";
  }
  if (
    normalizedKey.includes("to_trinh_hang_hoa") ||
    normalizedKey.includes("proposal_good") ||
    normalizedKey.includes("goods_proposal")
  ) {
  ...
  if (
    normalizedKey.includes("bao_gia_dich_vu") ||
    normalizedKey.includes("quotation_service") ||
    normalizedKey.includes("service_quotation")
  ) {
    return "Báo giá dịch vụ";
  }
  if (
    normalizedKey.includes("bao_gia_hang_hoa") ||
    normalizedKey.includes("quotation_good") ||
    normalizedKey.includes("goods_quotation")
  ) {
    ...
    return (
      <div className="flex items-center gap-1 px-1 py-1 rounded-[4px]">
        <button
          type="button"
          onClick={onToggle}
          className="size-6 shrink-0 text-[#393740] cursor-pointer"
          title={expanded ? "Thu gọn" : "Mở rộng"}
        >
          <ChevronDown className={`size-5 transition-transform ${expanded ? "" : "-rotate-90"}`} />
        </button>
        <span className="w-[150px] shrink-0 px-0.5 text-sm font-semibold text-[rgba(47,43,61,0.9)]">
          {getFieldLabel(field.label || field.id)}
        </span>
          ...
          {editing ? (
            isLongField(field) ? (
              <textarea
                ...
                className="w-full resize-y rounded border border-[#3f81ea] px-2 py-1 text-sm outline-none"
              />
            ) : (
              <input
                value={draft}
                ...
                className="h-8 w-full rounded border border-[#3f81ea] px-2 text-sm outline-none"
              />
            )
          ) : (
            <span
              className={`${
                isDocumentTypeField(field) || isLongField(field)
                  ? "block whitespace-pre-wrap break-words"
                  : "block truncate"
              } px-0.5 text-sm text-[rgba(47,43,61,0.9)]`}
              title={displayValue}
            >
              {displayValue}
            </span>
        ...
        <span className="rounded-[4px] bg-[#e8fadf] px-2.5 py-1 text-[13px] font-medium text-[#28c76f]">
          {field.confidence}%
        </span>
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
            editing ? onCommit() : onStartEdit();
          }}
          className={`p-1 cursor-pointer ${editing ? "text-[#28c76f]" : "text-slate-600 hover:text-[#3f81ea]"}`}
          title={editing ? "Xác nhận" : "Chỉnh sửa"}
        >
  ...
  return (
    <div
      ...
      className={`rounded-[4px] transition-colors ${
        isWarning
          ? `border-[0.5px] ${isLow ? "border-[#ff4c51] bg-[#ffdbdc]" : "border-[#ff9f43] bg-[#ffecd9]"} px-1 py-1`
          : `px-1 py-1 ${selected ? "bg-blue-50/70" : "hover:bg-slate-50"}`
      }`}
    >
      <div className="flex items-center gap-1 pl-7">
          ...
          {isWarning || editing ? (
            isLongField(field) ? (
              <textarea
                ...
                onFocus={onStartEdit}
                className={`w-full resize-y rounded-[4px] border bg-white px-1 py-1 text-sm text-[rgba(47,43,61,0.9)] outline-none ${
                  isLow ? "border-[#ff4c51]" : isMedium ? "border-[#ff9f43]" : "border-[#3f81ea]"
                }`}
              />
            ) : (
              <input
                ...
                onChange={(e) => editing && onDraftChange(e.target.value)}
                onFocus={onStartEdit}
                className={`h-8 w-full rounded-[4px] border bg-white px-1 text-sm text-[rgba(47,43,61,0.9)] outline-none ${
                  isLow ? "border-[#ff4c51]" : isMedium ? "border-[#ff9f43]" : "border-[#3f81ea]"
                }`}
              />
            )
        ...
        <div
          className={`shrink-0 ${
            isWarning ? "flex w-[132px] items-center" : "flex w-[100px] items-center justify-end gap-3"
          }`}
        >
          {isWarning && (
            <div className="flex w-[32px] items-center justify-center">
              <IconAlertTriangle className={`size-4 shrink-0 ${isLow ? "text-[#ff4c51]" : "text-[#ff9f43]"}`} />
            </div>
          )}
          <div className={`${isWarning ? "flex w-[100px] items-center gap-3 px-0.5" : "contents"}`}>
            <span
              className={`rounded-[4px] px-2.5 py-1 text-[13px] font-medium ${
                isLow
                  ? "bg-[#ffbfc1] text-[#ea5455]"
                  : isMedium
                  ? "bg-[#ffecd9] text-[#ff9f43]"
                  : "bg-[#e8fadf] text-[#28c76f]"
              }`}
            >
              {field.confidence}%
            </span>
            {isWarning || editing ? (
              <button
                ...
                className="p-1 text-[#28c76f] cursor-pointer"
                title={editing ? "Xác nhận" : "Chỉnh sửa trường cảnh báo"}
              >
              ...
              <button
                ...
                className="p-1 text-slate-600 hover:text-[#3f81ea] cursor-pointer"
                title="Chỉnh sửa"
              >
      ...
      {isWarning && edited && (
        <div className="flex items-center gap-1 pl-7 pt-1 text-[11px] text-slate-500">
...
export function DocumentDetailPage({
    ...
    try {
      ...
      toast.error(
        error instanceof ApiError
          ? `Bóc tách LLM thất bại (${error.status}): ${error.message}`
          : "Không thể bóc tách dữ liệu bằng LLM."
      );
    } finally {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      setExpandedLineItems((current) =>
        current.size || !latest.lineItems?.length ? current : new Set([latest.lineItems[0].id])
      );
      ...
      attempts += 1;
      const isProcessing =
        latest.status === "ocr" || latest.status === "processing" || latest.status === "queued";
      const needsOcrResult =
        latest.status === "stored" && (!latest.fields || latest.fields.length === 0);
  ...
  async function executeFieldUpdate(f: ExtractedField, newValue: string, reason?: string) {
    try {
      ...
      toast.error(
        error instanceof ApiError
          ? `Cập nhật thất bại (${error.status}): ${error.message}`
          : "Lỗi khi cập nhật trường."
      );
    }
  }
  ...
  async function handleConfirmDocument() {
    try {
      ...
      toast.error(
        error instanceof ApiError
          ? `Xác nhận thất bại (${error.status}): ${error.message}`
          : "Không thể xác nhận tài liệu."
      );
    }
  }
    ...
    try {
      ...
      const pollTimer = setInterval(async () => {
        try {
          ...
          toast.error(
            error instanceof ApiError
              ? `Kiểm tra OCR thất bại (${error.status}): ${error.message}`
              : "Không thể kiểm tra tiến trình OCR."
          );
        }
      }, 1000);
    } catch {
    ...
    <div className="min-h-screen bg-[#f8f7fa] text-slate-800 flex flex-col p-6 font-sans">
      ...
      <div className="bg-white rounded-[6px] border border-slate-200 p-4 shadow-[0px_4px_18px_0px_rgba(75,70,92,0.10)] space-y-4 flex-1">
        ...
        <div className="flex items-center gap-3 mb-5">
          <button
            onClick={onBack}
            className="p-1 hover:bg-slate-200 rounded-[6px] text-[#393740] transition-colors cursor-pointer"
            title="Quay lại"
          >
        ...
        <div className="flex items-center justify-between pb-1 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-1.5 rounded-[6px] border border-[#3f81ea] bg-white text-xs font-medium text-[#3f81ea] hover:bg-blue-50 disabled:opacity-50 transition-colors shadow-2xs cursor-pointer"
            >
              {llmRequesting ? "Đang bóc tách..." : "Bóc tách LLM"}
            </button>
            <button className="px-4 py-1.5 rounded-[6px] border border-slate-300 bg-white text-xs font-medium text-[#393740] hover:bg-slate-50 transition-colors shadow-2xs cursor-pointer">
              Lưu tạm
            </button>
            <button
              onClick={handleConfirmDocument}
              disabled={confirmed}
              className="px-4 py-1.5 rounded-[6px] bg-[#ff4c51] text-white text-xs font-semibold hover:bg-[#ea5455] disabled:opacity-50 transition-colors shadow-2xs cursor-pointer"
            >
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center justify-between mb-3 px-0.5 text-xs text-slate-700">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 bg-[#f0eff4] hover:bg-[#e4e3e8] rounded-[6px] text-[#5d586c] transition-colors cursor-pointer"
                  title="Xoay 90°"
                >
                  <RotateCw className="size-4" />
                </button>
                <button
                  onClick={() => setZoom((z) => getNextZoom(z, "out"))}
                  className="p-2 bg-[#f0eff4] hover:bg-[#e4e3e8] rounded-[6px] text-[#5d586c] transition-colors cursor-pointer"
                  title="Thu nhỏ"
                >
                ...
                <button
                  onClick={() => setZoom((z) => getNextZoom(z, "in"))}
                  className="p-2 bg-[#f0eff4] hover:bg-[#e4e3e8] rounded-[6px] text-[#5d586c] transition-colors cursor-pointer"
                  title="Phóng to"
                >
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={handleScan}
                  disabled={scanning}
                  className="p-1.5 hover:bg-slate-100 rounded-[6px] text-[#5d586c] transition-colors cursor-pointer"
            ...
            <div className="bg-white rounded-[8px] border border-slate-200 flex flex-col overflow-hidden shadow-2xs relative">
              <button
                ...
                className="absolute top-3 right-3 z-10 p-2 bg-[#f0eff4] hover:bg-[#e4e3e8] rounded-[6px] text-[#5d586c] transition-colors shadow-2xs cursor-pointer"
          ...
          <div className="flex flex-col space-y-4">
              ...
              <div className="space-y-1 max-h-[580px] overflow-y-auto pr-1 custom-scrollbar">
                ...
                {ocr?.fullText && (
                  <details className="border-t border-slate-200 pt-3">
                    <summary className="cursor-pointer text-sm font-semibold text-[#393740]">
                      Nội dung OCR đầy đủ
                    </summary>
                    <textarea
                      readOnly
                      value={ocr.fullText}
                      rows={8}
                      className="mt-2 w-full resize-y rounded border border-slate-200 bg-slate-50 p-2 text-sm leading-relaxed text-slate-700"
                    />
                  </details>
                )}
              </div>
            ...
            <div className="bg-white rounded-[8px] border border-slate-200 p-5 shadow-[0px_3px_12px_0px_rgba(47,43,61,0.14)] space-y-4">
              ...
              >
                ...
                <button className="p-1 text-slate-500 hover:text-slate-700 transition-transform cursor-pointer">
                  <svg
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[13px] font-semibold text-[#393740]">
                              {log.editor || "Người dùng"}
                            </span>
                          </div>
          ...
          <div className="bg-white rounded-[6px] shadow-[0px_3px_12px_0px_rgba(47,43,61,0.14)] p-6 w-[531px] space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                ...
                className="px-4 py-1.5 rounded-[6px] bg-[#7e8299] text-white text-xs font-semibold hover:bg-[#6c7086] cursor-pointer"
              >
                Huỷ
              </button>
              <button
                ...
                className="px-4 py-1.5 rounded-[6px] bg-[#3f81ea] text-white text-xs font-semibold hover:bg-[#2b6bd8] disabled:opacity-50 cursor-pointer"
              >
                Lưu chỉnh sửa
              </button>
            </div>
