...
export function CreateBusinessPlanModal({
  ...
  const handleSubmit = async (status: "Chờ phê duyệt" | "Lưu nháp") => {
    ...
    try {
      ...
      const created = await businessPlanApi.createBusinessPlan({
        id,
        code,
        title,
        partner,
      ...
      <div className="relative flex max-h-[90vh] w-full max-w-2xl flex-col rounded-[8px] bg-white shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          ...
          <button
            ...
            className="flex size-8 items-center justify-center rounded-[6px] text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors cursor-pointer"
          >
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-100 bg-[#f8f7fa] px-6 py-3.5">
          <button
            ...
            onClick={() => onOpenChange(false)}
            disabled={loading}
            className="h-9 rounded-[6px] border border-[#dbdade] bg-white px-4 text-[13px] font-medium text-[#5d586c] transition-colors hover:bg-slate-50 cursor-pointer"
          >
            Hủy
          </button>
          <button
            ...
            disabled={loading}
            className="h-9 rounded-[6px] border border-slate-300 bg-white px-4 text-[13px] font-medium text-[#2f2b3d] transition-colors hover:bg-slate-100 cursor-pointer"
          >
            Lưu nháp
          </button>
          <button
            ...
            disabled={loading}
            className="inline-flex h-9 items-center gap-1.5 rounded-[6px] bg-[#ff4c51] px-5 text-[13px] font-medium text-white shadow-xs transition-colors hover:bg-[#e64449] cursor-pointer"
          >
