...
import { IconEye, IconFilter, IconMoreVertical, IconPencil } from "../../components/icons";
import { type ContractItem } from "../../api/mocks/contractMock";
import { contractApi } from "../../api/contractApi";
import { ApiError } from "../../core/types/common.types";
import type { ContractActivityDto, ContractSummaryDto } from "../../core/types/contract.types";
...
export function ContractListPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      contractApi.getContractStats(),
      contractApi.getContractActivity(),
      ...
      if (list.status === "fulfilled") {
        ...
        setTotalPages(1);
        toast.error(
          list.reason instanceof ApiError ? list.reason.message : "Không thể tải danh sách hợp đồng"
        );
      }
    ...
    <div className="min-h-full w-full space-y-6 bg-[#f8f7fa] p-6 text-[#393740]">
      <PageHeader
        ...
        action={
          <button
            ...
            className="inline-flex h-[46px] items-center gap-2 rounded-[6px] bg-[#ff4c51] px-5 text-[13px] font-medium text-white shadow-sm hover:bg-[#e64449] cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 gap-4 rounded-[6px] bg-white p-5 shadow-[0_2px_8px_rgba(47,43,61,0.08)] md:grid-cols-[160px_160px_180px_180px_1fr_44px]">
        ...
        <div className="flex items-end">
          <button
            ...
            className="flex size-9 items-center justify-center rounded-[6px] border border-slate-200 bg-white text-[#5d586c] transition-colors hover:bg-slate-50 hover:text-[#393740] cursor-pointer"
          >
                ...
                filteredContracts.map((item) => (
                  <tr key={item.id} className="h-[42px] hover:bg-slate-50/70">
                    ...
                    <td className="max-w-[130px] px-3 py-2">
                      <span className="line-clamp-2">{item.goodsName}</span>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1 text-slate-500">
                        <button className="p-1 hover:text-[#3f81ea] cursor-pointer" title="Xem chi tiết">
                          <IconEye className="size-4" />
                        </button>
                        <button className="p-1 hover:text-[#3f81ea] cursor-pointer" title="Chỉnh sửa">
                          <IconPencil className="size-4" />
                        </button>
                        <button className="p-1 hover:text-[#3f81ea] cursor-pointer" title="Tùy chọn">
      ...
      <section className="rounded-[6px] bg-white p-5 shadow-[0_2px_8px_rgba(47,43,61,0.12)]">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button className="inline-flex items-center gap-1 text-[13px] text-[#3f81ea] hover:text-[#2f6fd1] cursor-pointer">
            Xem tất cả nhật ký
        ...
        <div>
          {apiActivities
            ...
            .map(({ icon: Icon, color, text, actor, time }, index, list) => (
              <div key={`${text}-${time}`} className="grid grid-cols-[36px_1fr_160px] gap-3">
                <div className="relative flex justify-center">
                  ...
                  {index < list.length - 1 && (
                    <span className="absolute top-8 h-10 border-l border-slate-200" />
                  )}
                </div>
