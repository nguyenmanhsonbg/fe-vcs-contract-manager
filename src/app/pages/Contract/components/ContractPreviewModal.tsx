...
export function ContractPreviewModal({ isOpen, onClose, contractData }: ContractPreviewModalProps) {
  ...
  const downloadPreviewPdf = async () => {
    const url = URL.createObjectURL(
      await contractApi.previewContract({ ...contractData, cleanPlaceholderDecorations: true })
    );
    const link = document.createElement("a");
    link.href = url;
    link.download = `${contractData.contractNumber || "contract"}_preview.pdf`;
    link.click();
  ...
  useEffect(() => {
    ...
    async function fetchPreviewPdf() {
      ...
      setError(null);
      try {
        const blob = await contractApi.previewContract(contractData);
        if (isMounted) {
    ...
    <div className="fixed inset-0 z-50 bg-[#f8f7fa] p-4">
      <div className="flex h-full w-full flex-col gap-4 overflow-hidden rounded-[6px] border border-slate-200 bg-white p-5 shadow-[0_4px_18px_rgba(75,70,92,0.1)]">
        <div className="flex shrink-0 items-center gap-3">
          <button
            onClick={onClose}
            className="rounded-[6px] p-1 text-[#2f2b3d] transition-colors hover:bg-slate-100 cursor-pointer"
            title="Quay lại"
          >
        ...
        <div
          ...
          {...pan}
          className={`min-h-0 flex-1 overflow-auto rounded-[6px] border border-slate-200/80 bg-slate-100/50 p-6 shadow-[inset_0_1px_3px_rgba(47,43,61,0.08)] select-none ${
        ...
        >
          {loading && (
            <div className="flex flex-col items-center justify-center space-y-3 text-zinc-500">
              <Loader2 className="w-8 h-8 animate-spin text-[#3f81ea]" />
          ...
          {!loading && !error && pdfUrl && (
            <div className="w-full h-full flex justify-center">
              <div
                className="flex min-h-full min-w-full justify-center"
                style={{ minWidth: `${Math.max(100, zoom)}%` }}
              >
                <DocumentCanvas
                  zoom={zoom}
                  page={page}
