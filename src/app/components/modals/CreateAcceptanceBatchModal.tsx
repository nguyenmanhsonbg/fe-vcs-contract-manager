...
import { X, Calendar, Plus, FileText, CheckCircle } from "lucide-react";
import { AcceptanceContractDetail } from "../../data/acceptanceMock";
...
export function CreateAcceptanceBatchModal({
  ...
  const nextPeriodNo = (contract.activePeriods?.length || 0) + 1;
  ...
  const totalBatchValue = contract.items.reduce((acc, item) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const itemsPayload = contract.items.map((item) => ({
        itemId: item.id,
      ...
      await docApi.createAcceptancePeriod(contract.id, {
        ...
        <form onSubmit={handleSubmit} className="max-h-[75vh] overflow-y-auto p-6 space-y-5 text-xs">
          ...
          <div className="flex items-center justify-end gap-2 border-t border-slate-200 pt-4">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-[6px] bg-[#ff4c51] px-5 py-2 text-xs font-medium text-white shadow-2xs hover:bg-[#e64449] disabled:opacity-50 transition-colors"
            >
