...
export function TaskDetailModal({
  ...
  const detailRows = [
    ...
    {
      label: "Trạng thái",
      value: <StatusBadge status={currentData.status || "processing"} label={currentData.status === "processing" ? "Đang đánh giá" : undefined} />,
    },
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="pt-4 flex items-center gap-3">
            <button
              type="button"
              onClick={() => onOpenChange(false)}
              className="flex-1 py-2.5 px-4 rounded-md border border-slate-300 bg-white text-slate-700 font-medium text-xs hover:bg-slate-50 transition-colors shadow-2xs text-center"
            >
              Xem chi tiết
            </button>
            <button
              ...
              className="flex-1 py-2.5 px-4 rounded-md bg-[#ff4c51] hover:bg-[#e64449] text-white font-medium text-xs shadow-sm transition-colors text-center"
            >
              Mở hồ sơ
            </button>
          </div>
