...
import { AcceptanceContractDetail, AcceptanceDocument } from "../../data/acceptanceMock";
...
export function UploadAcceptanceDocumentModal({
  ...
  const [period, setPeriod] = useState<string>(contract.activePeriods[0]?.name || "Đợt 1");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await docApi.uploadAcceptanceDocument(contract.id, {
        fileName: file.name,
        type: docType,
        period,
      });
